...
function ChipReadback({
  ...
  function handleDimension(field: 'lengthCm' | 'widthCm', raw: string) {
    const n = parseInt(raw, 10)
    onChange({ ...visionResult, [field]: isNaN(n) || n <= 0 ? undefined : n })
  }
    ...
    <div className="space-y-3 rounded-2xl border border-border bg-card/60 px-4 py-3.5">
      ...
      <div>
        <p className="mb-1.5 text-xs text-muted-foreground">
          Dimensions
          {(visionResult.lengthCm || visionResult.widthCm) && (
            <span className="ml-1.5 text-[10px] text-muted-foreground/60">(AI estimate — adjust if off)</span>
          )}
        </p>
        <div className="flex gap-2">
          {(
            ...
            <label key={field} className="flex flex-1 flex-col gap-1">
              <span className="text-[10px] font-medium text-muted-foreground/70">{label}</span>
              <div className="flex items-center overflow-hidden rounded-lg border border-border bg-card focus-within:border-primary/60 focus-within:ring-1 focus-within:ring-primary/20">
                <input
                  type="number"
                  inputMode="numeric"
                  min={50}
                  max={2000}
                  placeholder="—"
                  value={visionResult[field] ?? ''}
                  onChange={(e) => handleDimension(field, e.target.value)}
                  className="w-full bg-transparent px-2.5 py-1.5 text-[12px] text-foreground placeholder:text-muted-foreground/40 focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                />
                <span className="shrink-0 pr-2.5 text-[10px] text-muted-foreground/60">cm</span>
              </div>
            </label>
          ))}
        </div>
        <p className="mt-1.5 text-[10px] leading-relaxed text-muted-foreground/50">
          Rough is fine — your designer will verify on site. Leave blank if you&apos;re not sure.
        </p>
      </div>
    </div>
  )
